...
########## ## Odd-numbered minor release versions (e.g. 4.1) are unsuitable for production use #####
...
MDB_VERSION=`curl "${REDHAT_URL}" | sed --silent "s/^.*href='\([0-9]\+\.[0-9]*[02468]\)'.*$/\1/p" | sort --version-sort | tail --lines=1 `
